...
def fill_taxes(d, config):
    ...
    config keys:
        year, computation, computation_ny, computation_ny_recapture, computation_nyc,
    ...
    computation_ny_recapture = config['computation_ny_recapture']
    ...
    class MortgageInterestDeductionWorksheet(Worksheet):
        ...
        def build(self):
            ...
            summary_info[f"{self.key} 11 Qualified loan limit for {year}"] = self.d[11]
    ...
    class CapitalLossCarryoverWorksheet(Worksheet):
        ...
        def build(self):
            ...
            if prior_year['schedule_d_net_long_term'] < 0:  # it's ok to repeat
                ...
                # loss as positive number
                summary_info[f"{self.key} 13 Long-term capital loss carryover for {year}"] = self.d[13]
    ...
    class QualifiedDividendsCapitalGainTaxWorksheet(Worksheet):
        ...
        def build(self):
            ...
            self.d[11] = self.d[9]
            self.d[12] = self.d[10] - self.d[11]
            self.d[13] = config['qualified_div_20pct']  # single
