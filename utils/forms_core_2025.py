...
from utils.forms_functions import (
    get_main_info,
    computation_2025 as computation,
    computation_2025_ny as computation_ny,
    computation_2025_ny_recapture as computation_ny_recapture,
    computation_2025_nyc as computation_nyc,
)
...
def fill_taxes_2025(d, output_2024=None):
    if output_2024 is not None:
        states_2024, worksheets_all_2024 = output_2024
    else:
        states_2024, worksheets_all_2024 = None, None
    ...
    standard_deduction = 15_000  # if single or married filing separately
    ...
    class Form6251(Form):
        ...
        def build(self):
            ...
            # Part II
            self.push_to_dict('5_value', 88_100)  # see exceptions
            self.push_to_dict('6_value', max(0, self.d.get('4_value', 0) - self.d.get('5_value', 0)))
            if self.d.get('6_value') > 0:
                # line 7
                self.push_to_dict(
                    '7_value',
                    self.d['6_value'] * 0.26 if self.d['6_value'] < 232_600 else
                    self.d['6_value'] * 0.28 - 4_652
                )
    ...
    class MortgageInterestDeductionWorksheet(Worksheet):
        ...
        def build(self):
            ...
            summary_info[f"{self.key} 11 Qualified loan limit for 2025"] = self.d[11]
            ...
            if self.d[11] >= self.d[12]:
                ...
                summary_info[f"{self.key} 13 All Interest Deductible for 2025"] = self.d[13]
                return

            self.d[14] = round(self.d[11] / self.d[12], 3)
            self.d[15] = self.d[13] * self.d[14]
            summary_info[f"{self.key} 15 Deductible Home Mortgage Interest for 2025"] = self.d[15]
            self.d[16] = self.d[13] - self.d[15]
            summary_info[f"{self.key} 16 Personal (not Deductible) Interest for 2025"] = self.d[16]
    ...
    class CapitalLossCarryoverWorksheet(Worksheet):
        ...
        def build(self):
            if states_2024 is None:
                return
            self.d[1] = states_2024[k_1040]['15']
            self.d[2] = max(0., states_2024[k_1040sd]['21'])  # sign flip
            self.d[3] = max(0., self.d[1] + self.d[2])
            self.d[4] = min(self.d[2], self.d[3])
            if states_2024[k_1040sd]['7'] < 0:
                self.d[5] = max(0, -states_2024[k_1040sd]['7'])
                self.d[6] = max(0, states_2024[k_1040sd]['15'])
                self.d[7] = self.d[4] + self.d[6]
                self.d[8] = max(0., self.d[5] - self.d[7])  # enter this in D 6
                # loss as positive number
                summary_info[f"{self.key} 8 Short-term capital loss carryover for 2025"] = self.d[8]
            if states_2024[k_1040sd]['15'] < 0:  # it's ok to repeat
                self.d[9] = max(0., -states_2024[k_1040sd]['15'])
                self.d[10] = max(0., states_2024[k_1040sd]['7'])
                self.d[11] = max(0., self.d[4] - self.d[5])
                self.d[12] = self.d[10] + self.d[11]
                self.d[13] = max(0., self.d[9] - self.d[12])  # enter in D 14
                # loss as positive number
                summary_info[f"{self.key} 13 Long-term capital loss carryover for 2025"] = self.d[13]
