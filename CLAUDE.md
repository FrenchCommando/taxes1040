...
- Form 6781: supports both `Contract1256` (individual contracts) and `Realized1256`/`Unrealized1256` (aggregate broker amounts) in 1099 input. The 40/60 split (lines 8/9) flows directly to Schedule D lines 4 (short-term) and 11 (long-term), bypassing Form 8949. When only 1256 contracts exist (no regular trades), Form 8949 is omitted. No .keys/.fields for 2025 yet (computation only, no PDF fill).
