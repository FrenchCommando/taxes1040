...
- `make_pdf_output` filters out `custom_missing_*` keys (used in `.fields` to consume unused PDF annotations) with a warning log, rather than reporting them as unmatched computation keys.
